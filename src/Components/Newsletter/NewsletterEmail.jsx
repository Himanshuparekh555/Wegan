...
class NewsletterEmail extends React.Component {  
    ...
    render(){
        ...
        <Box sx={{margin:'10px auto'}} width={{xs:'100%',md:'65%',lg:'65%'}} >
            <Alert            
            sx={{
                padding:'0px 10px',
                borderRadius:12,
            }} 
            variant="outlined" severity={(this.state.isvalid ? "success" : "error")}>
                {this.state.message}
            </Alert>
         </Box>
        ) : (
        ...
        return (
            ...
            <Box backgroundColor={{xs:'#FAFAFA',md:'#FAFAFA',lg:'#ffffff'}}  sx={{ display:"flex",alignItems:"center",borderRadius:10,height:60}}>
                ...
                <Button type="submit" onClick={() => this.onSubmit()} className="btn-theme w-100" sx={{ height:60,borderRadius:10,}}>{"Subscribe"}</Button>
                </Box>
                </Box>
                <Box>
                    {messageTemplate}    
